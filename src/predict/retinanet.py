...
def _load_model(model_path: Path, device: torch.device) -> torch.nn.Module:
    # Trained with num_classes=2 (background + hematoma)
...
def _predict_one(
    ...
    *,
    ...
    """
    Returns lines: 'class x y w h conf' (normalized to resized dims).
    """
    img = Image.open(str(image_path)).convert("RGB")
    img_resized, _ratio, (W, H) = _resize_max_side(img, resize_number)
...
def predict_kfold(cfg: Dict) -> Dict[str, int]:
    ...
    cfg:
      ...
      weights_dir_template: "P:/BruiseDet_Repo/outputs/training/{strategy}/RetinaNet"
      aug_data_root:        "P:/BruiseDet_Repo/data/augmented/{strategy}"
      out_dir_template:     "P:/BruiseDet_Repo/outputs/predictions/{strategy}/RetinaNet"
      folds: null or [1,2,...]
    ...
    weights_root = Path(cfg.get("weights_dir_template", r"P:/BruiseDet_Repo/outputs/training/{strategy}/RetinaNet").format(strategy=strat))
    aug_root     = Path(cfg.get("aug_data_root", r"P:/BruiseDet_Repo/data/augmented/{strategy}").format(strategy=strat))
    out_root     = Path(cfg.get("out_dir_template", r"P:/BruiseDet_Repo/outputs/predictions/{strategy}/RetinaNet").format(strategy=strat))
    ...
    print(f"[retinanet|predict] strategy={strat}")
    print(f"[retinanet|predict] weights: {weights_root}")
    print(f"[retinanet|predict] val data root: {aug_root}")
    print(f"[retinanet|predict] out: {out_root}")
    ...
    for k in folds:
        ...
        print(f"[retinanet|predict] k{k}: {len(imgs)} images | weights={wpath}")
        ...
        print(f"[retinanet|predict] k{k}: wrote {written} files -> {save_dir}")
